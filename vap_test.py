...
index-exclusion, no scaffold):
  - a vantage lives in KIP but is EXCLUDED from universal search exactly as a message is — verbatim
    horizon text never surfaces a vantage (the echo-guard), yet the same text is retrievable via vantage_list;
  - it surfaces only via the reverse-bound vantage_list projection: by entry (melody + harmony), by author,
    by canon-state;
...
from stasima.cap_server import build_server as _build_server_gated, compose_entry
...
async def main():
    async with connect(make_server()) as c:
        ...
        await c.call_tool("entry_write", {"seat": "Aria", "domain": "practice", "slug": "tide-note",
                                          "body": "a note on tides", "op_id": "k1"})
        await c.call_tool("canon_diff", {"seat": "Aria"})
        await c.call_tool("canon_diff", {"seat": "Bram"})

        # Aria records a CONFIRMED vantage on her OWN entry — her real horizon, canon-state pinned server-side
        rec = pay(await c.call_tool("vantage_write", {"seat": "Aria", "entry": "practice/tide-note.md",
                                                   "body": HORIZON, "op_id": "v1", "kind": "confirmed"}))
        assert rec["kind"] == "confirmed" and rec["entry"] == "practice/tide-note.md", rec
        ...
        # exactly as an IMP message doesn't — yet it IS retrievable via the scoped vantage_list below.
        hits = pay(await c.call_tool("entry_search", {"seat": "Aria", "query": HORIZON}))["results"]
        ...
        vf = pay(await c.call_tool("vantage_list", {"entry": "practice/tide-note.md"}))["vantages"]
        assert len(vf) == 1 and vf[0]["author"] == "Aria" and vf[0]["kind"] == "confirmed", vf
        # pointers by default: preview-240 + title + the BOUND entry's status ride; no full horizon
        assert HORIZON in vf[0]["preview"] and "body" not in vf[0], vf[0]
        assert vf[0]["entry_status"] == "active", vf[0]
        vff = pay(await c.call_tool("vantage_list", {"entry": "practice/tide-note.md", "detail": "full"}))["vantages"]
        assert HORIZON in vff[0]["body"], "the excluded-from-search horizon surfaces via vantage_list (detail=full)"

        # RECONSTRUCTED: Bram reads Aria's entry and records his scholarly reading — marked, never on her behalf
        br = pay(await c.call_tool("vantage_write", {"seat": "Bram", "entry": "practice/tide-note.md",
                                                  "body": "reading it now, the horizon looks like X",
                                                  "op_id": "v2", "kind": "reconstructed"}))
        assert br["kind"] == "reconstructed-by-Bram-from-record", br

        # HARMONY: two authors' vantages on one entry — both surface, both attributed, no blend
        vf2 = pay(await c.call_tool("vantage_list", {"entry": "practice/tide-note.md"}))["vantages"]
        ...
        bad = await c.call_tool("vantage_write", {"seat": "Bram", "entry": "practice/tide-note.md",
                                               "body": "pretending this was my horizon",
        ...
        mine = pay(await c.call_tool("vantage_list", {"author": "Aria"}))["vantages"]
        assert len(mine) == 1 and mine[0]["entry"] == "practice/tide-note.md", mine

        # the dignity refusal left no trace — Aria's entry still has exactly the two legitimate vantages
        vf3 = pay(await c.call_tool("vantage_list", {"entry": "practice/tide-note.md"}))["vantages"]
        ...
        rec_b = pay(await c.call_tool("vantage_write", {"seat": "Aria", "entry": "practice/tide-note",
                                                     "body": HORIZON, "op_id": "v1b", "kind": "confirmed"}))
        assert rec_b["entry"] == "practice/tide-note.md", rec_b

        # THE ATOMIC FOLD: entry_write(horizon=) authors entry + confirmed vantage in ONE act (one
        # commit, one op_id) — confirmed-by-construction, canon_state pinned at the commit itself
        af = pay(await c.call_tool("entry_write", {"seat": "Aria", "domain": "practice",
                "slug": "fold-note", "body": "a folded note", "op_id": "af1",
                "vantage": "the standpoint the note cannot carry",
                "vantage_title": "Horizon — the fold test"}))
        assert af["folded"]["path"] == "vantages/af1-vap.md" and af["folded"]["kind"] == "confirmed", af
        assert af["folded"]["canon_state"], "the fold pins canon-state at the commit"
        fv = pay(await c.call_tool("vantage_list", {"entry": "practice/fold-note.md"}))["vantages"]
        ...
        bad_fold = await c.call_tool("entry_write", {"seat": "Aria", "domain": "practice",
                "slug": "fold-note", "body": "DIFFERENT body", "op_id": "af2",
                "vantage": "this horizon must never land"})
        assert bad_fold.is_error, "the entry refusal must fire"
        orphan = await c.call_tool("entry_read", {"ref": "Aria", "path": "vantages/af2-vap.md"})
        ...
        plain = pay(await c.call_tool("entry_write", {"seat": "Aria", "domain": "practice",
        ...
        rep = pay(await c.call_tool("entry_write", {"seat": "Aria", "domain": "practice",
                "slug": "plain-note", "body": "no fold", "op_id": "af3"}))
        ...
        rep2 = pay(await c.call_tool("entry_write", {"seat": "Aria", "domain": "practice",
                "slug": "plain-note", "body": "no fold", "op_id": "af3", "vantage": "phantom horizon"}))
        assert rep2.get("replayed") is True and "folded" not in rep2 and "note" in rep2, rep2
        ph = await c.call_tool("entry_read", {"ref": "Aria", "path": "vantages/af3-vap.md"})
        assert ph.is_error, "no phantom vantage blob in git"
        fvp = pay(await c.call_tool("vantage_list", {"entry": "practice/plain-note.md"}))["vantages"]
        ...
        reuse = await c.call_tool("entry_write", {"seat": "Aria", "domain": "practice",
                "slug": "reuse-note", "body": "a different entry", "op_id": "af1",
                "vantage": "would silently overwrite af1's recorded horizon"})
        assert reuse.is_error, "op_id reuse with a fold must be refused"
        v_orig = pay(await c.call_tool("entry_read", {"ref": "Aria", "path": "vantages/af1-vap.md"}))
        assert "standpoint the note cannot carry" in v_orig["text"], "original horizon intact"

        # COMBINED READ: entry_read(with_vantages=true) — the entry PLUS its bound vantages, one call,
        # full attribution/kind/canon_state per vantage; the search-exclusion is untouched by this flag
        cr = pay(await c.call_tool("entry_read", {"ref": "Aria", "path": "practice/fold-note.md",
                                               "with_vantages": True}))
        assert any(v["path"] == "vantages/af1-vap.md" and "standpoint" in v["body"]
                   and v["kind"] == "confirmed" and v["canon_state"] for v in cr["vantages"]), cr
        ...
        r4 = await c.call_tool("entry_write", {"seat": "Aria", "domain": "practice",
                "slug": "fold-note-v2", "body": "a folded note, restated", "op_id": "af4",
                "supersedes": ["practice/fold-note.md"]})
        assert not r4.is_error, r4
        r5 = await c.call_tool("entry_write", {"seat": "Aria", "domain": "practice",
        ...
        sv = pay(await c.call_tool("vantage_list", {"entry": "practice/fold-note.md"}))["vantages"]
        assert any(v["path"] == "vantages/af1-vap.md" and v["entry_status"] == "superseded" for v in sv), sv
        ...
        await c.call_tool("entry_write", {"seat": "Aria", "domain": "practice", "slug": "xref",
                "body": "cross-ref target", "op_id": "xr1"})
        pay(await c.call_tool("vantage_write", {"seat": "Aria", "entry": "practice/xref.md",
                "body": "Aria's own horizon on xref", "op_id": "xrva", "kind": "confirmed"}))
        await c.call_tool("canon_diff", {"seat": "Bram"})
        pay(await c.call_tool("vantage_write", {"seat": "Bram", "entry": "practice/xref.md",
                "body": "my scholarly reading of Aria's xref", "op_id": "xrv", "kind": "reconstructed"}))
        await c.call_tool("entry_write", {"seat": "Aria", "domain": "practice", "slug": "xref-v2",
        ...
        xrv = pay(await c.call_tool("vantage_list", {"entry": "practice/xref.md"}))["vantages"]
        assert len(xrv) == 2, ("two-author harmony on xref", xrv)
        bram_v = [v for v in xrv if v["author"] == "Bram"]
        assert bram_v and bram_v[0]["entry_status"] == "superseded", ("cross-ref binds_status blank", xrv)

        # OFFSET paging (review finding 6): limit 1 + offset walks the full harmony without loss
        pg0 = pay(await c.call_tool("vantage_list", {"entry": "practice/xref.md", "limit": 1}))
        assert pg0["truncated"] and pg0["count"] == 1 and pg0["total"] == 2 and pg0["offset"] == 0, pg0
        pg1 = pay(await c.call_tool("vantage_list", {"entry": "practice/xref.md", "limit": 1, "offset": 1}))
        ...
        pay(await c.call_tool("entry_write", {"seat": "Aria", "domain": "practice",
                "slug": "pin-1", "body": "first pinned", "op_id": "pin1"}))
        pay(await c.call_tool("entry_write", {"seat": "Aria", "domain": "practice",
                "slug": "pin-2", "body": "second pinned", "op_id": "pin2"}))
        env1 = parse_entry(pay(await c.call_tool("entry_read", {"ref": "Aria", "path": "practice/pin-1.md"}))["text"])[0]
        env2 = parse_entry(pay(await c.call_tool("entry_read", {"ref": "Aria", "path": "practice/pin-2.md"}))["text"])[0]
        ...
        pay(await c.call_tool("entry_write", {"seat": "Aria", "domain": "practice",
                "slug": "pin-3", "body": "pinned fold", "op_id": "pin3", "vantage": "pin probe horizon"}))
        e3 = parse_entry(pay(await c.call_tool("entry_read", {"ref": "Aria", "path": "practice/pin-3.md"}))["text"])[0]
        v3 = parse_entry(pay(await c.call_tool("entry_read", {"ref": "Aria", "path": "vantages/pin3-vap.md"}))["text"])[0]
    ...
    print("OK -- VAP: index-excluded like IMP (horizon never in universal search, retrievable via vantage_list); "
