...
"""Regenerate the Aous suite encoding (one-folder packaging) from LANDED canon.

Canon governs: this script reads the dock sources from the deployment's git repo — never from
drafts — and composes skills/aous/: SKILL.md (the arrival road with its dock index rewritten
...
Run after any land that touches a suite source:
    ...
    python skills/gen_aous.py <path-to-bare-repo> [out-dir]

Aous is the fifth river — the 0.1.5 (wire-lean) contract. The one-folder shape carries forward
...
MANIFEST = "meta/suites/aous.md"

DESCRIPTION = ("Participating in a Stasima knowledge server — the Aous suite (one folder: the "
