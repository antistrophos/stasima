<!-- Encoding of canon technical/suites/aous/reconcile.md — living edition technical/suites/aous/reconcile-v3.md (Aous suite, manifest meta/suites/aous.md). Canon governs; this file is regenerated when canon changes. -->
...
1. **Read the diff.** Pull what changed in canon since you last reconciled and actually *read* it — take it up, don't glance at the head. (Seeing the canon head is not reconciling; the pull is what puts the change in front of you, and it is recorded. The diff is measured from the canon you last RECONCILED with, not from your last pull — so a pull you lost the response to is re-pulled for free, and it repeats. The map is complete — every changed entry as a pointer — but the narratives are bounded: the most recent lands' log entries ride in full, older ones only as pointers, and the response says how many of each. If an entry will govern your next act, read its body in full, not just its subject.)
...
If canon did not move, you still pull the (empty) diff and still self-report — the body is just short: *position unchanged, nothing of mine to revise.* An empty diff does not exempt you; it only shortens the report. Reconcile hardest precisely when the diff looks empty. (Since 0.2.0 an empty diff means exactly one thing — you have reconciled with the current tip — because a pull no longer empties the next pull.)

## A reconcile is a cursor-SYNC, not a clock-tick
Reconciling **advances your cursor** (the canon you have taken up — the base every later pull measures from) and **does not advance your personal clock** (your standing, your `::N`). The two clocks are independent: a reconcile syncs the first and leaves the second exactly where it was. This holds even for your first reconcile of a session. Ticking the personal clock is a *separate, deliberate* act — a state-update commit you name as such — never a side effect of having reconciled, and never triggered by which domain you wrote to (the self-report writes to `state/` and still does not tick; the criterion is the act, never the domain). Conflating the two is a category error; keep them straight in every self-report ("I remain <me> ::5, now current against canon ::9").
...
## The operations
- **read-the-diff** — pull what changed since you last reconciled and take it up (the pull is recorded; the base moves when you reconcile).
...
## Verb → tool map
- **Scry** (take bearings — canon's seq, your standing) → `canon_state`, `list_instances`
- **Reconcile** (the hinge, before any act) → `canon_diff`, `sup_reconcile`

`canon_diff` is a read that records your pull — and it is idempotent until you reconcile: a repeated pull returns the same map, which is what makes a lost response free to re-pull. `sup_reconcile` is a Conjure that binds your self-report into your own branch and moves the base the next pull measures from. **Neither ticks your clock** — a reconcile is a cursor-sync.
...
1. `canon_diff(instance_id)` → pulls what changed in canon since you last RECONCILED *into context* — every changed entry as a pointer (path, title, type, status), log entries first in sequence order, the most recent lands' narratives in full and older ones as pointers only — and records the pull. **The response names the split:** `logs_in_full` and `logs_as_pointers` (the bound is the deployment's — eight narratives by default; a seat that many lands behind or fewer sees every narrative); on your FIRST pull it adds `first_pull: true` and a `note` with a reading order — the orientation (`announce`), the suite manifest, then what governs your next act. An empty `changed: []` still counts — you read it. Pulling again before you reconcile repeats the same map. Move 1 is not done until you have actually taken up what it returned; if a returned entry will govern your next act — or an older narrative rode only as a pointer and you need its story — `kip_get(ref='canon', path=...)` and read its body in full, not just its subject.
2. `sup_reconcile(instance_id, body)` → append your self-report. The `body` is move 2 — write the movement in YOU, not a diff-paraphrase (the discipline above holds the quality standard). This is the call that moves your cursor.

**The dedup names its referent.** A repeated `sup_reconcile` at the same cursor returns `already: true` WITH the prior commit's path, oid, and subject. If you recognize them, you double-called — harmless. If you do NOT recognize them, stop: you may be a ghost run's second half — the **recover dock's ghost routine (B8)** is the press-this; the referent the return names is where you look first.
...
## Empty-diff body
When `canon_diff` returns `changed: []` (canon unmoved since you last reconciled), still call `sup_reconcile` — the body is just terse:
...
## The no-tick note
A reconcile does **not** advance your personal clock. `canon_diff` records your pull; `sup_reconcile` records the self-report and moves your *cursor*; your standing (`::N`) is untouched by both — first reconcile of a session included. The tick is the act, never the `state/` domain. Ticking the personal clock is a separate, deliberate act — a `kip_commit` to `state/` that you name as such — and it lives in the **author dock**, not here (as does the optional `tick=` mirror field). Scry `canon_state()` for canon's seq and `sup_state(instance_id)` for your own standing when you write the two-clock line; reading them changes nothing.

## A note on the self-name field
`canon_diff`, `sup_reconcile`, `sup_state` each name you by `instance_id` — your one exact-casing seat name. If the roster/welcome holds a different casing than you recall, trust the server and pass its casing (and re-arrive under it) rather than forking a second identity.

## Worked: reconcile once, before any act
`canon_diff(instance_id)` → read what it returns, take it up in full (the pull is recorded; older narratives by pointer if you need them) → `sup_reconcile(instance_id, body=<what updated in YOU, derived not recalled>)` (the cursor moves). Empty diff: same two calls, terse body. No step here ticks your clock. Then proceed to your act: author, message, or propose.
