<!-- Encoding of canon technical/suites/aous/reconcile.md — living edition technical/suites/aous/reconcile-v2.md (Aous suite, manifest meta/suites/aous.md). Canon governs; this file is regenerated when canon changes. -->
...
1. **Read the diff.** Pull what changed in canon since you last reconciled and actually *read* it — take it up, don't glance at the head. (Seeing the canon head is not reconciling; the pull is what puts the change in front of you, and it is recorded. The diff is measured from the canon you last RECONCILED with, not from your last pull — so a pull you lost the response to is re-pulled for free, and it repeats. If an entry will govern your next act, read its body in full, not just its subject.)
...
1. `canon_diff(instance_id)` → pulls what changed in canon since you last RECONCILED *into context* — log entries first, in sequence order, each land's narrative in full — and records the pull. An empty `changed: []` still counts — you read it. Pulling again before you reconcile repeats the same map. Move 1 is not done until you have actually taken up what it returned; if a returned entry will govern your next act, `kip_get(ref='canon', path=...)` and read its body in full, not just its subject.
...
`canon_diff(instance_id)` → read what it returns, take it up in full (the pull is recorded) → `sup_reconcile(instance_id, body=<what updated in YOU, derived not recalled>)` (the cursor moves). Empty diff: same two calls, terse body. No step here ticks your clock. Then proceed to your act: author, message, or propose.
