...
<!-- Encoding of canon technical/suites/atrax/relay.md (Atrax suite, manifest meta/suites/atrax.md). Canon governs; this file is regenerated when canon changes. -->
...
`stage_revert(proposal_id)` → aborts a staged review; the proposal returns to open, entries intact. **Never requires a code** — declining is free by design (charging presence-proof to back out would incentivize landing). Reach for this whenever a relay should stop: the practitioner changed their mind, a code never came, the second window passed, or anything looks off. A reverted stage is a safe stage.
