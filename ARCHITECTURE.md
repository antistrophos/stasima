...
- **Identity is a declared name**, recorded as provenance — attribution, not attestation. The base threat model is *loss, not forgery*: a single practitioner with cooperating instances. **Session binding** (see *The HTTP era* below) now pins a connection to a seat name — transport-sourced, not payload-claimed — so a mistyped or injected identity is refused rather than silently misattributed; it is the anti-spoofing hardening the earlier posture deferred, arrived early because one shared HTTP process made connection-blind identity a real hazard.
...
Two transports, one server. **stdio** (`transport = "stdio"`) is the original shape: the client spawns one server process per chat, strictly sequential, no shared concurrency. **http** (`transport = "http"`) runs ONE continuous server that serves the whole fleet — clients reach `http://<host>:<port>/mcp` over streamable HTTP, where each conversation is its own transport session. Because a desktop client may multiplex many chats onto one process (a "trunk"), and clients that demand a local connector can front the service with a thin stdio→HTTP bridge (`mcp-proxy`), the identity granularity that matters is the **session**, not the OS process.

**Session binding — the identity pin (TOFU, SSH-shaped).** A connection binds to a seat name from one of three sources, strongest first: **pinned** (`STASIMA_INSTANCE` in the server definition's env — pre-seeded, no first-use moment), **port-durable** (`STASIMA_PORT` — a learned binding persisted as append-only `port_binding` events; the audit ledger *is* the running config, and the cockpit clears it), and **sticky** (the default: the first identity-claiming write on a session binds it for that session's life). The source is always the transport (env or the streamable-HTTP session object, held weakly), never a payload the model authors — that is what makes it anti-spoofing rather than one more trusted assertion. Three modes govern a mismatch (`STASIMA_BINDING`): **strict** (refuse, secure-by-default), **witness** (proceed but stamp `authored_via=<bound>` into the envelope and audit — for a connection that legitimately acts as more than one name), **off** (the explicit, server-owned downgrade). Reads are never guarded; the relay verbs carry no identity parameter, so `approved_by` stays code-proven. Under HTTP each conversation binds independently — the trunk-collision problem dissolves at the transport.
