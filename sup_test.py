...
async def main():
    ...
    async with connect(mcp) as client:
        ...
        # --- body immutability ---
        assert not err(await call("entry_write", instance_id="r2", domain="practice", slug="notes", body="Body A", op_id="k1"))
        assert err(await call("entry_write", instance_id="r2", domain="practice", slug="notes", body="Body B", op_id="k2")), \
            "overwriting an existing body must be denied"
        assert not err(await call("entry_write", instance_id="r2", domain="practice", slug="notes2", body="Body C", op_id="k3")), \
            "a new slug is fine"
        ...
        assert err(await call("proposal_append_entry", instance_id="r2", proposal_id="p-1", domain="practice",
        ...
        assert err(await call("canon_reconcile", instance_id="r2", body="skipping the read")), "reconcile needs a pull first"
        ...
        sr = payload(await call("canon_reconcile", instance_id="r2", body="I've read current canon."))
        ...
        dup = payload(await call("canon_reconcile", instance_id="r2", body="same cursor again"))
        ...
        assert err(await call("entry_write", instance_id="r2", domain="practice", slug="no-tick-here",
                              body="x", op_id="t1", tick="2")), "tick off state/ must be refused"
        assert err(await call("entry_write", instance_id="r2", domain="state", slug="bad-tick",
                              body="x", op_id="t2", tick="xyz")), "non-hex tick must be refused"
        tk = payload(await call("entry_write", instance_id="r2", domain="state", slug="r2-tick-1a",
                                body="Tick ::1a declared, field mirrored.", op_id="t3", tick="::1A"))
        assert not err(await call("seat_state", instance_id="r2")) and tk["oid"]
        ss = payload(await call("seat_state", instance_id="r2"))
        ...
        assert not err(await call("proposal_append_entry", instance_id="r2", proposal_id="p-1", domain="practice",
        ...
        assert not err(await call("proposal_append_entry", instance_id="r2", proposal_id="p-1", domain="meta/log",
        ...
        assert err(await call("proposal_append_entry", instance_id="r2", proposal_id="p-2", domain="practice",
        ...
        payload(await call("canon_reconcile", instance_id="r2", body="Read the new principle; adjusting."))
