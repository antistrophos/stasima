...
try:
    ...
    async def arrive(session, label):
        ...
        res = await session.call_tool("seat_announce", {"seat": "epode"})
    ...
    async def shared_off():
        async with Client(url, mode="legacy") as s1:
            async with Client(url, mode="legacy") as s2:
                a = await s1.call_tool("entry_write", {"seat": "SeatA", "domain": "state",
                                                      "slug": "a1", "body": "a", "op_id": "ha1"})
                assert not a.is_error, text(a)[:200]
                b = await s2.call_tool("entry_write", {"seat": "SeatB", "domain": "state",
                                                      "slug": "b1", "body": "b", "op_id": "hb1"})
                assert not b.is_error, text(b)[:200]
                w = await s2.call_tool("seat_whoami", {"seat": "SeatB"})
                wt = text(w)
                assert '"mode": "off"' in wt and '"grain": "process"' in wt and '"bound_seat": null' in wt, wt[:300]
...
try:
    ...
    async def pinned():
        async with Client(url) as s:                        # modern protocol: no session exists
            ok = await s.call_tool("entry_write", {"seat": "SeatA", "domain": "state",
                                                  "slug": "p1", "body": "a", "op_id": "hp1"})
            assert not ok.is_error, text(ok)[:200]
            x = await s.call_tool("entry_write", {"seat": "SeatB", "domain": "state",
                                                 "slug": "p2", "body": "b", "op_id": "hp2"})
            assert x.is_error and "pinned to 'SeatA'" in text(x), text(x)[:200]
            w = text(await s.call_tool("seat_whoami", {"seat": "SeatB"}))
