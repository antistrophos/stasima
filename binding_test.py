...
Binding at process grain (the SSH-shaped identity pin, with port-security sticky learning):
  ...
  2. the message_send identity alias: seat= (canonical) + seat= (deprecated twin); conflict
...
from stasima.cap_server import build_server as _build_server_gated, compose_entry, parse_entry, port_bindings
...
async def main():
    ...
    async with connect(mcp) as client:
        ...
        ok = payload(await client.call_tool("entry_write", {"seat": "Verso", "domain": "state",
                                                           "slug": "one", "body": "x", "op_id": "b1"}))
        assert ok["author"] == "Verso", ok
        bad = await client.call_tool("entry_write", {"seat": "Recto", "domain": "state",
        ...
        a1 = payload(await client.call_tool("message_send", {"seat": "Verso", "recipients": ["Recto"],
                                                         "subject": "s", "body": "b", "op_id": "m1"}))
        assert a1["from"] == "Verso", a1
        typo = await client.call_tool("message_send", {"seat": "Recto", "recipients": ["Verso"],
                                                   "subject": "the original typo shape",
                                                   "body": "b4", "op_id": "m4"})
        assert typo.is_error and "pinned to 'Verso'" in err_text(typo),             "the forgery-by-typo class must refuse under a pinned binding"
        print("2. message_send: seat is the one identity parameter; the forgery typo refused")

        stage_props = tools["proposal_stage"].input_schema.get("properties", {})
        land_props = tools["proposal_land"].input_schema.get("properties", {})
        assert "seat" not in stage_props and "sender" not in stage_props, stage_props
        assert "seat" not in land_props and "sender" not in land_props, land_props
    ...
    async with connect(mcp2) as client:
        r = payload(await client.call_tool("entry_write", {"seat": "Recto", "domain": "state",
    ...
    async with connect(mcp4) as client:
        w0 = payload(await client.call_tool("seat_whoami", {"seat": "Recto"}))
        assert w0["binding"] == {"mode": "strict", "grain": "process", "bound_seat": None,
                                 "source": None, "match": None}, w0
        first = payload(await client.call_tool("entry_write", {"seat": "Recto", "domain": "state",
        ...
        w1 = payload(await client.call_tool("seat_whoami", {"seat": "Recto"}))
        assert w1["binding"]["bound_seat"] == "Recto" and \
               w1["binding"]["source"] == "process" and w1["binding"]["match"] is True, w1
        again = payload(await client.call_tool("entry_write", {"seat": "Recto", "domain": "state",
                                                              "slug": "st2", "body": "b", "op_id": "s2"}))
        assert again["author"] == "Recto", again             # same identity keeps working
        other = await client.call_tool("entry_write", {"seat": "Verso", "domain": "state",
    ...
    async with connect(mcp3) as client:
        r1 = payload(await client.call_tool("entry_write", {"seat": "Anyone", "domain": "state",
                                                           "slug": "open", "body": "z", "op_id": "o1"}))
        r2 = payload(await client.call_tool("entry_write", {"seat": "Someone", "domain": "state",
        ...
        w = payload(await client.call_tool("seat_whoami", {"seat": "Anyone"}))
    ...
    async with connect(mcp5a) as client:
        payload(await client.call_tool("entry_write", {"seat": "Recto", "domain": "state",
    ...
    async with connect(mcp5b) as client:
        w = payload(await client.call_tool("seat_whoami", {"seat": "Recto"}))
        assert w["binding"] == {"mode": "strict", "grain": "process", "bound_seat": "Recto",
    ...
    async with connect(mcp5c) as client:
        r = payload(await client.call_tool("entry_write", {"seat": "Verso", "domain": "state",
