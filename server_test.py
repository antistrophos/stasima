...
orient -> author (indexed inline) -> entry_search -> read own trail -> propose -> check status
...
async def main():
    ...
    async with connect(mcp) as client:   # default: tool errors come back as CallToolResult(isError=True)
        ...
        mp = payload(await client.call_tool("entry_list", {"ref": "research-2"}))   # 0.1.5: absorbs my_perspective
        got = payload(await client.call_tool("entry_read", {"ref": "research-2", "path": "practice/durability-notes.md"}))
        ...
        le = payload(await client.call_tool("entry_list", {"ref": "canon"}))
        assert all("path" in e and "status" in e for e in le["entries"]), "entry_list entries are enriched pointers"
        ...
        lf = payload(await client.call_tool("entry_list", {"ref": "canon", "path": "practice/"}))
        ...
        ps = payload(await client.call_tool("proposal_list", {}))["statuses"]["p-1"]   # 0.1.5: absorbs proposal_status
        assert ps["status"] == "open", ps
        print("proposal_preview:", cp, "| lifecycle:", ps["status"])
        ...
        cp2 = payload(await client.call_tool("proposal_preview", {"proposal_id": "p-1"}))
        ...
        cpx = payload(await client.call_tool("proposal_preview", {"proposal_id": "p-x"}))
        ...
        one = payload(await client.call_tool("thread_list", {"thread": "weave-test"}))
        ...
        one = payload(await client.call_tool("term_list", {"term": "weft"}))
        ...
        print("proposal_close: creator lane + terminal-for-seats + lifecycle listing OK")
        # the boundary meter: every git crossing is counted and timed at the _run chokepoint
        pf = payload(await client.call_tool("server_stats", {}))
        ...
        env2, _ = parse_entry(payload(await client.call_tool("entry_read",
        ...
        live = payload(await client.call_tool("entry_read", {"ref": "research-2", "path": "practice/durability-notes.md"}))
        ...
        corpse = payload(await client.call_tool("entry_read",
        ...
        miss = await client.call_tool("entry_read", {"ref": "research-7", "path": "practice/durability-v2.md"})
        ...
        before_fer = fer()
        allf = payload(await client.call_tool("message_unread_count", {}))
        ...
        print("entry_write into messages/ denied:", denied, "| denial audit-logged:", denial_logged)
        ...
        assert denied and denial_logged, "authz seam denies + logs a message sent via entry_write"
